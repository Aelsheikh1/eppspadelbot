...
export default function GameDetails() {
  ...
  useEffect(() => {
    // Real-time updates for game document
    const gameRef = doc(db, 'games', id);
    const unsubscribe = gameRef && id ? onSnapshot(gameRef, (docSnap) => {
      if (docSnap.exists()) {
  ...
  const handleCloseGame = async () => {
    ...
    try {
      ...
      try {
        ...
        setFeedback('Game closed and notification sent to all participants');
      } catch (emailErr) {
        console.error('Failed to send email:', emailErr);
        setFeedback('Game closed but failed to send notifications');
      }
    } catch (err) {
      console.error('Error closing game:', err);
      setFeedback('Failed to close game: ' + err.message);
    }
  };
  ...
  const handleReopenGame = async () => {
    ...
    try {
      ...
      setFeedback('Game reopened successfully');
    } catch (err) {
      setFeedback('Failed to reopen game: ' + err.message);
    }
  };
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4, mb: 4 }}>
      {/* Feedback Snackbar for closure/reopen actions */}
      <Snackbar
        open={!!feedback}
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            ...
            <List>
              ...
              <ListItem>
                <ListItemText primary="Duration" secondary={`${game.duration || 1.5} hours`} />
              </ListItem>
              <ListItem>
